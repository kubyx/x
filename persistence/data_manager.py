...
from core import system_management # alias ve makro çalıştırmak için
...
def run_alias(alias):
    ...
    if alias in config.aliases:
        # system_management modülünden fonksiyonu çağır
        return system_management.run_shell_command(config.aliases[alias])
    else:
...
def run_macro(name):
    ...
    for cmd in config.macros[name]:
        # system_management modülünden fonksiyonu çağır
        results.append(system_management.run_shell_command(cmd))
    return "\n".join(results)
